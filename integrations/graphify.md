...
Output is written to `graphify-out/` in the current directory: `graph.json` (structured node/edge graph — nodes are functions/files/classes, edges are call/import relationships), `GRAPH_TREE.html` (interactive visualisation), and `manifest.json` (repo metadata). `/brain-ingest-repo` reads `graph.json` and writes the digest to the wiki automatically.
