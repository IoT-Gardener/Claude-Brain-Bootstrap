...
install_command "end-session.md"
...
if grep -q "$MARKER" "$GLOBAL_CLAUDE" 2>/dev/null; then
...
else
    {
        ...
        echo "## Brain"
        echo ""
        echo "**REQUIRED: Before doing anything else in a new session — before reading"
        echo "files, before exploring the codebase, before answering questions — locate"
        echo "the brain and run \`/brain-query\` with the user's request as the query.**"
        echo ""
        echo "Also run \`/brain-query\` again mid-session whenever you detect a significant"
        echo "context switch: a different project, codebase, domain, or topic that the"
        echo "opening query did not cover. Use your judgement — a new question within the"
        echo "same topic does not need a re-query; switching from TTRPG lore to homelab"
        echo "planning, or from one repo to another, does."
        echo ""
        echo "To locate the brain: if \`\$BRAIN_ROOT\` is set in the environment, use that"
        echo "path. Otherwise walk up from the current working directory looking for a"
        echo "\`wiki/index.md\` file. If no brain is found, carry on normally without"
        echo "mentioning it."
        echo ""
        echo "The brain is the primary source of context for architecture, decisions,"
        echo "domain knowledge, and ongoing work. Only read individual source files when"
        echo "the brain's answer is insufficient or out of date."
        echo ""
        ...
        echo "/brain-log-session, /brain-librarian, /brain-lint, /end-session."
...
echo "  Commands:         ~/.claude/commands/brain-*.md + end-session.md"
